# Graphing QA stats
# see qa3 and qa4b_extract_wcs.R for data generation
...
# Water results
# acres with increase
# % of area with increase
# (also have rounded-tens percent changes)
...
#in/output folder
folder_qaz <- file.path("data", "qa", "qa_zonal")

stamp <- format(Sys.time(), "%Y%m%d")

### Data in --------------------------------------------------------------------

stats_p1 <- readr::read_csv(file.path(
  folder_qaz,
  "part1_fvs_percchange_stats_20260802.csv"
))

stats_p2 <- readr::read_csv(file.path(
  folder_qaz,
  "part2_fvs_biomass_stats_20260802.csv"
))

stats_p3 <- readr::read_csv(file.path(
  folder_qaz,
  "part3_waterpchange_stats_20260802.csv"
))
